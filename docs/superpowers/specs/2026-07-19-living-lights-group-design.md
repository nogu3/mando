...
- グループカードでの全メンバー状態集計。
- 展開状態の永続化。
- グループ操作後の展開内メンバータイルの追いつき取得（メンバー集計ポーリングをしない
  設計の帰結として、展開中のメンバー表示は古くなりうる。気になったら再訪で更新される）。
- 非展開メンバーの初回 state 取得の遅延ロード（現状は boot 時に全 light を 1 回読む。
  直列 exec の負荷が問題になったら展開時フェッチへ）。
