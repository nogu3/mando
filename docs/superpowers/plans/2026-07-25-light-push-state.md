...
- クライアント定数: `LIGHT_CATCHUP_MS = 2000`（既存・SSE 断時の fallback）、`LIGHT_SETTLE_MS = 2000`（新規・通信しない）
...
- [ ] mando 起動後、`curl -s localhost:8080/api/devices/living_lights/state` が
      `"source":"push"` で **~即答**すること（初回の再ベースライン read が済んだ後）
...
- [ ] 既に点いているライトの「つける」を押しても「反映中…」で固まらないこと（settle タイマー）
- [ ] `systemctl --user restart matd` → listener が再接続し、再ベースライン後に
      再び `"source":"push"` へ戻ること（backoff のログが出る）
